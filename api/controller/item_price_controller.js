...
exports.deleteItemPrice = async (req, res, next) => {
  try {
    await ItemPrice.sequelize.query(`DELETE FROM myan_market.item_price WHERE id = :itemPriceId`, {
      replacements: { itemPriceId: req.params.itemPriceId },
...
exports.getPriceHistoryOfAItem = async (req, res, next) => {
  try {
    ...
    const query = `
      SELECT item.name, item.unit, itemPrice.buy_price AS "buyPrice", 
      itemPrice.sell_price AS "sellPrice", itemPrice.buy_price_changes AS "buyPriceChanges", 
      itemPrice.sell_price_changes AS "sellPriceChanges", itemPrice.status, 
      itemPrice.created_datetime AS "createdDatetime", itemPrice.modified_datetime AS "modifiedDatetime"
